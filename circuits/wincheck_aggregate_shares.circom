...
// Once each player received encrypted shares from all other players
// each player can aggregate the shares they have 
template Wincheck_Aggregate_Shares(generator, num_players, bit_length) {
    ...
    signal output sum1;
    signal output sum2;
    // signal output message1[num_players];
    // signal output message2[num_players];
    ...
    for (var i = 0; i < num_players; i++) {
        ...
        adder1.numbers[i] <== elGamalDecrypter1[i].message[1];   // message [0] is discarded
        // message1[i] <== elGamalDecrypter1[i].message[1];
        ...
        adder2.numbers[i] <== elGamalDecrypter2[i].message[1];   // message [0] is discarded
        // message2[i] <== elGamalDecrypter2[i].message[1];
    }
