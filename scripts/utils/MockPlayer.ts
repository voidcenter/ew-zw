...
export class MockPlayer {
    ...
    async encryptAndShuffleDeck(aggKey: bigint, deckToMask: bigint[][], nPlayers: number): Promise<EncryptAndShuffleProof> {
        ...
        // these path are relative to the execution path
        const wasmFilename = `./circuits/zkshuffle_encrypt_and_shuffle_${nPlayers}.wasm`;
        const zkeyFilename = `./circuits/zkshuffle_encrypt_and_shuffle_${nPlayers}.zkey`;
    ...
    async decryptDeck(deckToUnmask: bigint[][], nPlayers: number): Promise<DecryptProof> {
        ...
        const wasmFilename = `./circuits/zkshuffle_decrypt_${nPlayers}.wasm`;
        const zkeyFilename = `./circuits/zkshuffle_decrypt_${nPlayers}.zkey`;
    ...
    async decryptMyCard(cardToUnmask: bigint[]): Promise<Role> {
        ...
        const groth16PO = await exportSolidityCallData(
            ...
            }, `./circuits/zkshuffle_decrypt_self.wasm`, `./circuits/zkshuffle_decrypt_self.zkey`)
        );
    ...
    async winCheck_createShares(nPlayers: number, shareValueOffset: bigint): Promise<WinCheck_CreateShares_Proof> {
        ...
        const wasmFilename = `./circuits/wincheck_create_shares_${nPlayers}.wasm`;
        const zkeyFilename = `./circuits/wincheck_create_shares_${nPlayers}.zkey`;

        // console.log(this.roleCardBeforeDecryption);
        // console.log(shares1);
        // console.log(shares2);
        ...
        const groth16PO = await exportSolidityCallData(
            await groth16.fullProve(data, wasmFilename, zkeyFilename)
        );
        console.log('public signals length = ', groth16PO.publicSignals.length);
    ...
    async winCheck_aggregateShares(nPlayers: number, 
                                   shareCiphertexts1: Tuple2n[], 
                                   shareCiphertexts2: Tuple2n[]): Promise<WinCheck_AggregateShares_Proof> {

        // generate witness and proof
        const wasmFilename = `./circuits/wincheck_aggregate_shares_${nPlayers}.wasm`;
        const zkeyFilename = `./circuits/wincheck_aggregate_shares_${nPlayers}.zkey`;
