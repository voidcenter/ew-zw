...
import { render_log_msg, Command, System } from '../web3/msgRenderer';
...
export async function check_triggers(state: any, pushMsgs: any) {
    // pushTermText('tic');
    // await refreshState(setState);

    // shuffle 
    if (   state.gameState == GameState.KEY_AGGREGATION 
        && state.shuffleState == GameState.KEY_AGGREGATION 
        && state.playerPubKeys.length == state.myPlayerIndex) {
      if (!start_agg_key) {
        start_agg_key = true;
        pushMsgs([<TerminalOutput><System>Submitting public key ... </System></TerminalOutput>]);
        await shuffle.aggregateKey();
      }
    }
    if (   state.gameState == GameState.KEY_AGGREGATION 
        && state.shuffleState == GameState.SHUFFLE 
        && state.nPlayers_encryptedAndShuffled == state.myPlayerIndex) {
      if (!start_ens) {
        start_ens = true;
        pushMsgs([<TerminalOutput><System>Submitting encrypted and shuffled role cards ... </System></TerminalOutput>]);
        await shuffle.ens(state);
      }
    }
    if (   state.gameState == GameState.KEY_AGGREGATION 
        && state.shuffleState == GameState.DECRYPT 
        && state.nPlayers_decrypted == state.myPlayerIndex) {
      if (!start_decrypt) {
        start_decrypt = true;
        pushMsgs([<TerminalOutput><System>Submitting decrypted role cards ... </System></TerminalOutput>]);
    ...
    if (   state.gameState == GameState.DAY_WINCHECK_CREATE 
        && state.secureAddState == GameState.DAY_WINCHECK_CREATE 
        && state.stillInGame[state.myPlayerIndex]) {
      if (!start_wincheck_create) {
        start_wincheck_create = true;
        pushMsgs([<TerminalOutput><System>Submitting vote secret shares ... </System></TerminalOutput>]);
    ...
    if (   state.gameState == GameState.DAY_WINCHECK_CREATE 
        && state.secureAddState == GameState.DAY_WINCHECK_AGGREGATE) {
      if (!start_wincheck_aggregate) {
        start_wincheck_aggregate = true;
        pushMsgs([<TerminalOutput><System>Submitting aggregated votes shares ... </System></TerminalOutput>]);
